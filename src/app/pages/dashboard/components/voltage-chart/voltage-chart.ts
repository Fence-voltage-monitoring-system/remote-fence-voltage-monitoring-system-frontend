...
export class VoltageChartComponent implements AfterViewInit, OnChanges {
  ...
  private draw(): void {
    ...
    const values = Array.from({ length: 21 }, (_, index) => this.device.status === 'offline' ? 0 : Math.max(0, Math.min(7, this.device.voltage + Math.sin(index / 2.4) * .55 + Math.cos(index / 1.8) * .18)));
    ...
    values.forEach((value, index) => { const x = padding + index * (width - padding - 10) / (values.length - 1), y = height - 18 - (value / 7) * (height - 35); index ? context.lineTo(x, y) : context.moveTo(x, y); });
    context.stroke();
  }
}
