...
interface AuditEvent { id:string; timestamp:string; actor:string; role:string; action:string; category:AuditCategory; resource:string; resourceId:string; description:string; result:AuditResult; severity:'info'|'warning'|'critical'; ip:string; client:string; changes:Change[]; }
...
export class AuditLogsPage {
  readonly events: AuditEvent[] = [
    {id:'AUD-00942',timestamp:'27 Jul 2026, 14:32:18',actor:'Nimal Perera',role:'Administrator',action:'Assigned device',category:'Device',resource:'North Gate Monitor',resourceId:'DVC-1024',description:'Assigned voltage monitor to section SEC-001.',result:'success',severity:'info',ip:'192.168.1.24',client:'Chrome 138 · Windows',changes:[{field:'Fence',before:'Unassigned',after:'Monaragala Elephant Protection Fence'},{field:'Section',before:'Unassigned',after:'SEC-001'},{field:'Status',before:'Offline',after:'Online'}]},
    {id:'AUD-00941',timestamp:'27 Jul 2026, 14:20:43',actor:'Kasun Silva',role:'Operations Manager',action:'Changed gateway',category:'Fence',resource:'Gal Oya East Protection Fence',resourceId:'FNC-004',description:'Reassigned fence to Monaragala Main Gateway.',result:'success',severity:'info',ip:'192.168.1.31',client:'Edge 138 · Windows',changes:[{field:'Gateway',before:'GTW-1002',after:'GTW-1004'}]},
    {id:'AUD-00940',timestamp:'27 Jul 2026, 13:55:09',actor:'Monitoring service',role:'System',action:'Gateway disconnected',category:'Gateway',resource:'Mihintale Field Gateway',resourceId:'GTW-1002',description:'Gateway stopped sending heartbeat messages.',result:'warning',severity:'warning',ip:'10.20.3.12',client:'System service',changes:[{field:'Status',before:'Online',after:'Offline'}]},
    ...
    {id:'AUD-00938',timestamp:'27 Jul 2026, 12:18:52',actor:'Nimal Perera',role:'Administrator',action:'Registered gateway',category:'Gateway',resource:'Spare Gateway 01',resourceId:'GTW-1001',description:'Added new gateway hardware to inventory.',result:'success',severity:'info',ip:'192.168.1.24',client:'Chrome 138 · Windows',changes:[{field:'Record',before:'Not present',after:'Created'}]},
    {id:'AUD-00937',timestamp:'27 Jul 2026, 11:46:31',actor:'Amali Fernando',role:'Field Officer',action:'Updated section',category:'Section',resource:'Western Boundary',resourceId:'SEC-003',description:'Updated section length after field inspection.',result:'success',severity:'info',ip:'192.168.1.46',client:'Chrome Mobile · Android',changes:[{field:'Length',before:'3.7 km',after:'3.9 km'}]},
    {id:'AUD-00936',timestamp:'27 Jul 2026, 10:22:05',actor:'Nimal Perera',role:'Administrator',action:'Disabled user',category:'User',resource:'Field Officer Account',resourceId:'USR-0018',description:'Disabled user access following role change.',result:'success',severity:'warning',ip:'192.168.1.24',client:'Chrome 138 · Windows',changes:[{field:'Account status',before:'Active',after:'Disabled'}]},
  ...
  search=''; category='all'; result='all'; severity='all'; dateRange='today'; selected?:AuditEvent;
  get filtered():AuditEvent[]{const q=this.search.trim().toLowerCase();return this.events.filter(e=>(this.category==='all'||e.category===this.category)&&(this.result==='all'||e.result===this.result)&&(this.severity==='all'||e.severity===this.severity)&&(!q||[e.id,e.actor,e.action,e.resource,e.resourceId,e.description,e.ip].some(v=>v.toLowerCase().includes(q))));}
  get failedCount(){return this.events.filter(e=>e.result==='failed').length} get securityCount(){return this.events.filter(e=>e.category==='Authentication'||e.category==='User').length} get changeCount(){return this.events.filter(e=>e.changes.length).length}
